...
The tool aims to replicate a typical dev workflow: plan, implement, QA (= goal), Ralph Wiggum loop until the goal is achieved, respond to PR comments on GitHub, etc.
...
- **[`extra/`](extra)** specifies opinionated, optional features (the Kanban
  ...
  skip the rest.
- **[`reference/`](reference)** is one complete, working implementation. It's the implementation we use daily internaly, with our own set of preferences. It's a *citation* for the spec, not the
  canonical version.
